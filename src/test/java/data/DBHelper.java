...
public class DBHelper {
    ...
    public static String getCreditId() {
        val ids = runn.query(conn,
                "SELECT id FROM credit_request_entity ORDER BY created;", hand);
        return ids.get(ids.size() - 1).getId();
    }

    @SneakyThrows
    public static String getCreditBankId() {
        val bankIds = runn.query(conn,
                "SELECT bank_id FROM credit_request_entity ORDER BY created;", hand);
        return bankIds.get(bankIds.size() - 1).getBankId();
    }

    @SneakyThrows
    public static String getCreditCreated() {
        val created = runn.query(conn,
                "SELECT created FROM credit_request_entity;", hand);
        return created.get(created.size() - 1).getCreated();
    }

    @SneakyThrows
    public static String getCreditStatus() {
        val statuses = runn.query(conn,
                "SELECT status FROM credit_request_entity ORDER BY created;", hand);
    ...
    public static String getOrderId() {
        val orderIds = runn.query(conn,
                "SELECT id FROM order_entity ORDER BY created;", hand);
    ...
    public static String getOrderCreated() {
        val created = runn.query(conn,
                "SELECT created FROM order_entity;", hand);
        return created.get(created.size() - 1).getCreated();
    }

    @SneakyThrows
    public static String getOrderCreditId() {
        val creditIds = runn.query(conn,
                "SELECT credit_id FROM order_entity ORDER BY created;", hand);
        return creditIds.get(creditIds.size() - 1).getCredit_id();
    }

    @SneakyThrows
    public static String getOrderPaymentId() {
        val paymentIds = runn.query(conn,
                "SELECT payment_id FROM order_entity ORDER BY created;", hand);
    ...
    public static String getPaymentId() {
        val ids = runn.query(conn,
                "SELECT id FROM payment_entity ORDER BY created;", hand);
        return ids.get(ids.size() - 1).getId();
    }

    @SneakyThrows
    public static int getPaymentAmount() {
        val amounts = runn.query(conn,
                "SELECT amount FROM payment_entity ORDER BY created;", hand);
    ...
    public static String getPaymentCreated() {
        val created = runn.query(conn,
                "SELECT created FROM payment_entity;", hand);
        return created.get(created.size() - 1).getCreated();
    }
    ...
    public static String getPaymentStatus() {
        val statuses = runn.query(conn,
                "SELECT status FROM payment_entity ORDER BY created;", hand);
        return statuses.get(statuses.size() - 1).getStatus();
    }
